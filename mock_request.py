...
def get_page(url_path):
    try:
        ...
        logging.error(f"{e}", extra={"url_path": url_path})
...
def get_page_every_time(url_path, time_white):
    logging.info("Start queries", extra={"url_path": url_path})
    while 1:
        get_page(url_path)
        white_time_minutes(time_white)        
...
if __name__ == "__main__":
    ...
    th1 = threading.Thread(target=get_page_every_time, args=(path_project, time_white))
